...
//                            514-สืบพยานจำเลย = บันทึกนัดสืบพยานจำเลย+บันทึกนัดสืบพยานวันที่
    ...
    @Override
    public void MASTER_DATA_ZUTBLMKTDFT(String date) {
        ParameterDetail parameterDetails = parameterDetailRepository.findByParameterAndCode("MASTERDATA_FILE", "17");
        String fileName = parameterDetails.getVariable1() + date + ".txt";
        String parameterCode = parameterDetails.getVariable2();
        ...
        try {
            ...
            if (file.isFile()) {
                ...
                saveOrUpdateZUTBLMKTDFT(dataList);
            }
    ...
    @SneakyThrows
    public void saveOrUpdateZUTBLMKTDFT(List<Map> data) {
        for (Map map : data) {
            String parameterCode = map.get("parameterCode").toString();
            String code = map.get("code").toString();
            String variable1 = (map.get("variable1") == null ? null : map.get("variable1").toString());
            ...
            List<ParameterDetail> detailCode = parameterDetails.stream()
                    .filter(value -> (value.getVariable1().equals(code) && value.getVariable2().equals(variable1) && value.getVariable3().equals(variable2)))
                    .collect(Collectors.toList());
            ...
            if (detailCode.isEmpty()) {
                ...
                if (parameterDetails.size() > 0) {
                    sequence = parameterDetails.size() + 1;
                }

                newParameterDetail = new ParameterDetail();
                newParameterDetail.setCode(String.valueOf(sequence));
            } else {
                newParameterDetail = parameterDetailRepository.findByPCodeAndPdCode(parameterCode, detailCode.get(0).getCode());
            }

            newParameterDetail.setVariable1(code);
            newParameterDetail.setVariable2(variable1);
            newParameterDetail.setVariable3(variable2);
            newParameterDetail.setVariable4(variable3);
            newParameterDetail.setVariable5(variable4);
            newParameterDetail.setVariable6(variable5);
            newParameterDetail.setVariable7(variable6);
            newParameterDetail.setVariable8(variable7);
            newParameterDetail.setVariable9(variable8);
